...
- **发布一律需要令牌**：关闭匿名发布。`POST /api/share` 与 `POST /api/upload` 都必须携带有效令牌（或站长主密码），凭证缺失返回 401 并附带申请入口；凭证无效不再被静默忽略、按匿名发布处理（详见 [ADR-0004](docs/adr/0004-token-required-for-publishing.md)）。
- **新增令牌申请页 `/apply`**：说明发布需要令牌、给出站长微信二维码，以及拿到令牌后在网页编辑器与 skill 中的用法；凭证不足时的 401 响应会带上这个地址。
- **编辑器**：分享弹层在无令牌时直接展示申请入口与令牌输入框，保存令牌前先向服务端校验有效性；令牌失效或被吊销时清除本地令牌并退回申请入口。
- **Skill**：`publish` 与其中的图片上传都要求 `WXMD_TOKEN`，缺令牌立即报错（不会上传到一半才失败），错误信息附带申请入口。
- **管理页修复**：`hidden` 属性此前被 `.overlay/.identity` 的自设 `display` 覆盖，导致登录页出现一个白色空白弹层（并露出不该出现的"退出"按钮），补回 `[hidden] { display: none !important; }`。
