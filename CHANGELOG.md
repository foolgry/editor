...
- **管理页**：项目列表新增「链接」列，直接展示并打开项目聚合链接（`/p/<id>`），便于把项目入口发给读者。
- **阅读页右侧大纲**：分享页（`/s/<id>`）与项目页（`/p/<id>/<sid>`）右侧新增文章大纲，取正文的 `h2/h3/h4`，默认展开、可收起（选择记在浏览器本地），随滚动高亮当前小节，点击平滑跳转；项目页切换文章后自动重建。正文右侧放不下面板时（宽屏阈值约 1200px / 带左侧目录的项目页约 1500px）整体不出现，以免压住正文；两个页面共用同一份 `frontend/outline.js`。
